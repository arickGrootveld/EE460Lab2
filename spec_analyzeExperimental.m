...
% lengthPrevData = length(BPFilter)-1;
% prevdata = zeros(FrameLength-lengthPrevData,1);
% startingValues = zeros(lengthPrevData,1);
...
%         for x = 1:lengthPrevData
%             startingValue = dot(flipud(BPFilter), [prevdata(x:lengthPrevData);data(1:x)]);
%             startingValues(x) = startingValue;
%         end
%         
%         restOfValues = filter(BPFilter,1,data);
%         data = [startingValues;restOfValues(lengthPrevData+1:FrameLength)];
...
%         prevdata = data(FrameLength-lengthPrevData);
